...
void dildit ( void )
{
    ...
    prv = ifract ; 
    fprintf ( stderr , "\rParsing ... %i%%" , ifract ) ; 
}
...
int rd ( void )
{
    ...
    if ( feof ( curinput ) ) {
        ...
        if ( inpf == curinput ) {
             undild () ; 
             if ( !useraccnumber )
                  fprintf ( stderr , "\nAccepted %i accessions, rejected %i" , accepted , rejected ) ; 
...
double doneedwunsch ( char * ap , char * bp ) 
{
    ...
    hei = strlen ( bp ) ; 
    if ( ! mademem ) {
         ( Stringcomptyp ** ) cellcost = ( Stringcomptyp ** ) loray ( 100 , 100 , sizeof ( Stringcomptyp ) ) ;
         mademem = 1 ; }
