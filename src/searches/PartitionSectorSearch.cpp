 ...
 */
 #include "PartitionSectorSearch.h"
...
PartitionSectorSearch::PartitionSectorSearch()
{
	curSector_size = 0;
	curSector_leaves = 0;
}
...
const char* PartitionSectorSearch::name() const
{
	return "Partition Sector Search";
}
 
int PartitionSectorSearch::BuildSectors(QTree* tree)
{
	mSectors.clear();
...
int PartitionSectorSearch::PostOrderTraversal(QNode* root)
{
...
void PartitionSectorSearch::AddNodeToCurrentSector(QNode* root)
{
	curSector_size++;
	if(root->nodeInfo()->leaf())
		curSector_leaves++;
...
void PartitionSectorSearch::StartNewSector()
{
...
void PartitionSectorSearch::addConstraints(QTree* tree,int sector_num)
{
	//return;
