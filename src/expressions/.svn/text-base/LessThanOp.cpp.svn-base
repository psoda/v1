...
LessThanOp::LessThanOp() : Operator() {
  return;
}
...
Literal& LessThanOp::operateOn(Environment* curEnv, Expression* operand1, Expression* operand2) {
  assert(operand1);
  assert(operand2);
...
string LessThanOp::getOpSymbol() const {
  return "<";
}
