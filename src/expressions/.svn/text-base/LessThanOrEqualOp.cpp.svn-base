...
LessThanOrEqualOp::LessThanOrEqualOp() : Operator() {
  return;
}
...
Literal& LessThanOrEqualOp::operateOn(Environment* curEnv, Expression* operand1, Expression* operand2) {
  assert(operand1);
  assert(operand2);
...
string LessThanOrEqualOp::getOpSymbol() const {
  return "<=";
}
