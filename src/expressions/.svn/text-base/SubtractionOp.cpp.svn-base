...
SubtractionOp::SubtractionOp() : Operator() {
  return;
}
...
Literal& SubtractionOp::operateOn(Environment* curEnv, Expression* operand1, Expression* operand2) {
  assert(operand1);
  assert(operand2);
...
string SubtractionOp::getOpSymbol() const {
  return "-";
}
