...
MultiplicationOp::MultiplicationOp() : Operator() {
  return;
}
...
Literal& MultiplicationOp::operateOn(Environment* curEnv, Expression* operand1, Expression* operand2) {
  assert(operand1);
  assert(operand2);
...
string MultiplicationOp::getOpSymbol() const {
  return "*";
}
