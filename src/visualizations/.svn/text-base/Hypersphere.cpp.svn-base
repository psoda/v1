...
list<ProjectedTree> Hypersphere::visualization()
{
	printf("In Hypersphere\n");
	list<ProjectedTree> treeList;
	
	int ntrees = mTrees.numTrees();
...
ProjectedTree Hypersphere::lookupTree(QTree* tree) const
{
		ProjectedTree point(3);
		PartitionTree* pt = new PartitionTree(tree);
...
ProjectedTree Hypersphere::lookupPartition(const Partition part) const
{
	return lookup(part);
}

ProjectedTree Hypersphere::lookup(const Partition part) const
{
	ProjectedTree point(3);
...
void Hypersphere::setup_tables(bool orthogonal)
{
