/**
 *	TreeStub.class is an instantiated object that reads in 
 *	data and places them in a UsageBean that
 * 	contains all of the operational decisions a user can 
 *	make regarding the uses of the Tree program.
 *	
...
public class TreeStub extends treesaap.Objects.Stub
{
	//Class Variables - Created Objects
	private TreeUsageBean bean;		//Bean created and populated by this class
	 ...
	 */
	public TreeStub(String user_settingsFileName) throws IOException
	{
	 ...
	 */
	public TreeStub(String user_settingsFileName, boolean errorWindow, String logFile) throws IOException
	{
		//create a new usage bean
		bean = new TreeUsageBean();
	...
	{
		//displayTCSWindow
		if(thisToken.equalsIgnoreCase("displayTCSWindow"))
		{
			bean.setDisplayTCSWindow(Boolean.valueOf(st.nextToken()).booleanValue());	
			return true;					
		}
		return false;
	...
	{		
		...
		outputString += "[ TCSTree User Input Values ]\n\n";
		outputString += "1.	displayTCSWindow	"+ bean.getDisplayTCSWindow() +"	[ Whether to display TCS window on a run ]\n\n";
