...
import treesaap.Data.TableBean;
...
public class DriverForPsoda extends Driver
{
	...
	private boolean firstTry = true;		//Whether this is the first initialization of driver class or not
		...
		{	
                if(driver == null) {
                       driver = new DriverForPsoda(false, ""); // "" is supposed to be a logfile. The bool tells whether the original gui is present.. in PSODA, it isn't
                       driver.initialize();
                }
            }
		catch(IOException e)
	...
	{		
		...
		try
		{
			...
			if(t == null || t.getNodeNames() == null)
				throw new Exception("Got Bad Tree");
			//set output dir
    ...
    {
    	//System.out.println("writing file at " + bean.getOutputDirectory() + "TranslatedSeqs.txt");
        String fileName = bean.getOutputDirectory() + "TranslatedSeqs.txt";
        try
        {
            PrintWriter outFile = new PrintWriter(new FileWriter(fileName));
            
            HashMap<String, String> singleLetterAas = new HashMap<String, String>();
            singleLetterAas.put("-", "-");
