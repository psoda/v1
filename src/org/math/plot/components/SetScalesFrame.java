...
public class SetScalesFrame extends JFrame implements WindowFocusListener, WindowListener {
	...
	public class ScalePanel extends JPanel implements StringPrintable {
		...
		private Choice title_field = new Choice();
		...
		public void update() {
			title = plotCanvas.getGrid().getAxe(numAxe).getLegend();// getLegend(numAxe);
			title_field.select(title);

			scaleType = plotCanvas.getAxesScales()[numAxe];
		...
		title_field.add("Parsimony Score");
		title_field.add("Time");
		title_field.add("Number of Trees");
...
*/
			title_field.addItemListener(new ItemListener() {
		...
		private void setTitle() {
			...
			// title_field.getText());
			plotCanvas.setAxeLabel(numAxe, title_field.getSelectedItem()/*getText()*/);
			Plot3DPanel.setAxisLabel(numAxe, title_field.getSelectedItem());
		}
		...
		private void setScale() {
			...
			try {
				plotCanvas.setAxeScale(numAxe, (log_check.isSelected()) ? (Base.LOGARITHM) : (Base.LINEAR));
			} catch (IllegalArgumentException iae) {
