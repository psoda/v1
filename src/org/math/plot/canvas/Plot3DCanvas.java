...
public class Plot3DCanvas extends PlotCanvas {
	...
	public void initBasenGrid(double[] min, double[] max) {
		initBasenGrid(min, max, new String[] { LINEAR, LINEAR, LINEAR }, new String[] { "Xaxis", "Yaxis", "Zaxis" });
	}
	...
	private static double[][] convertXY(double[][] XY) {
		if (XY.length == 3 && XY[0].length != 3)
			return mergeColumns(XY[0], XY[1], XY[2]);
		else
			return XY;
	}
	...
	public int addScatterPlot(String name, Color c, double[]... XY) {
		XY = convertXY(XY);
		return addPlot(new ScatterPlot(name, c, XY));
	}

	public int addLinePlot(String name, Color c, double[]... XY) {
		XY = convertXY(XY);
		return addPlot(new LinePlot(name, c, XY));
	}

	public int addBarPlot(String name, Color c, double[]... XY) {
		XY = convertXY(XY);
		return addPlot(new BarPlot(name, c, XY));
	}
