/**
 *	TCSStub.class is an instantiated object that reads in 
 *	data and places them in a UsageBean that
 * 	contains all of the operational decisions a user can 
 *	make regarding the uses of TCS algorithms.
 *	
...
class TCSStub extends DataConvert.Components.Stub
{
	//Class Variables - Created Objects
	private TCSUsageBean bean;		//Bean created and populated by this class
	 ...
	 */
	public TCSStub(String user_settingsFileName) throws IOException
	{
	 ...
	 */
	public TCSStub(String user_settingsFileName, boolean errorWindow, String logFile) throws IOException
	{
		//create a new usage bean
		bean = new TCSUsageBean();
	...
	{		
		...
		outputString += "[ TCS User Input Values ]\n\n";
		return outputString;
	}//writeData
